...
const userdispaydata = async () => {
    ...
    try {
        ...
        setData.map((v) => {
            let cinemaname = cinemaData.find((cinema) => cinema.id === v.cinema_id)?.cname || "Unknown Cinema";
            let moviename = movieData.find((movie) => movie.id === v.movie_id)?.mname || "Unknown Movie";
...
const handleEdit = async (id) => {
    ...
    document.getElementById("setno").value = updata.Seatobj[0].seat.length;
...
const handelAlldata = async () => {
    ...
    let start_date = new Date(fdata.createdAt) 
    let end_date = new Date(fdata.date)
